...
export default function MatchDetailModal({ match, teamStats, onClose }) {
  ...
  const comparisonMetrics = useMemo(() => {
    const metrics = [
      {
        label: "FIFA Ranking",
        barPercentage: homeStats.avg_gf + awayStats.avg_gf > 0
          ? (homeStats.avg_gf / (homeStats.avg_gf + awayStats.avg_gf)) * 100
          : 50 ,     
      ...
      },
      {
        ...
        barPercentage:
          homeStats.win_rate + awayStats.win_rate > 0
            ? (homeStats.win_rate / (homeStats.win_rate + awayStats.win_rate)) *
              100
            : 50,
      },
      {
        ...
        barPercentage:
          homeStats.wc_appearances + awayStats.wc_appearances > 0
            ? (homeStats.wc_appearances /
                (homeStats.wc_appearances + awayStats.wc_appearances)) *
              100
            : 50,
      },
    ];
  ...
  const scorelinePredictions = useMemo(() => {
    ...
    const lambdaH = Math.max(
      0.2,
      Math.min(4.5, homeStats.avg_gf * (awayStats.avg_ga / 1.3)),
    );
    const lambdaA = Math.max(
      0.2,
      Math.min(4.5, awayStats.avg_gf * (homeStats.avg_ga / 1.3)),
    );
        ...
        <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-5 flex flex-col justify-between">
            ...
            <div className="bg-[#080808]/40 border border-white/5 p-4 rounded-xl flex items-center justify-between text-center gap-3">
              ...
              <div className="flex-1 flex flex-col items-center gap-1.5 min-w-0">
                ...
                <span
                  className="text-[9px] uppercase tracking-wider text-text-muted-alt font-semibold font-noto"
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-[10px] text-text-muted-alt font-noto">
                  Machine learning simulated win/draw/loss expectations
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-center font-noto">
                <div
                  className={`p-2.5 rounded-xl border bg-dark-bg/60 transition-all ${predicted_winner === home ? "border-gold bg-gold-muted/10" : "border-white/5"}`}
                >
                  <span className="text-[9px] font-bold text-text-muted-alt uppercase tracking-wider block">
                    HOME WIN
                  </span>
                  <span className="text-xl font-bebas text-white-alt mt-1 block tracking-wide">
                    {formatPercent(home_win_prob)}
                  </span>
                </div>

                {draw_prob !== undefined && (
                  <div
                    className={`p-2.5 rounded-xl border bg-dark-bg/60 transition-all ${predicted_winner === "Draw" ? "border-gold bg-gold-muted/10" : "border-white/5"}`}
                  >
                    <span className="text-[9px] font-bold text-text-muted-alt uppercase tracking-wider block">
                      DRAW
                    </span>
                    <span className="text-xl font-bebas text-white-alt mt-1 block tracking-wide">
                      {formatPercent(draw_prob)}
                    </span>
                  </div>
                )}

                <div
                  className={`p-2.5 rounded-xl border bg-dark-bg/60 transition-all ${predicted_winner === away ? "border-gold bg-gold-muted/10" : "border-white/5"}`}
                >
                  <span className="text-[9px] font-bold text-text-muted-alt uppercase tracking-wider block">
                    AWAY WIN
                  </span>
                  <span className="text-xl font-bebas text-white-alt mt-1 block tracking-wide">
                    {formatPercent(away_win_prob)}
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-2.5">
              <div>
                ...
                <p className="text-[10px] text-text-muted-alt font-noto">
                  Top 5 calibrated scoreline outcomes based on Poisson
                  simulations
                </p>
              </div>
              ...
              <div className="space-y-2">
                {scorelinePredictions.map((pred, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="bg-[#080808]/40 border border-white/5 hover:border-gold-border/20 p-2 rounded-xl flex items-center justify-between gap-3 text-xs font-noto"
                    >
          ...
          <div className="space-y-3.5 bg-[#080808]/30 border border-white/5 p-4 rounded-xl flex flex-col justify-between">
            <div>
              ...
              <p className="text-[10px] text-text-muted-alt font-noto">
                Historical metrics and team performance comparison
              </p>
            </div>
            ...
            <div className="text-[9px] text-text-muted-alt/75 border-t border-white/5 pt-3 leading-relaxed mt-2 font-noto">
              ℹ️ <strong>Head-To-Head calculation note:</strong> Metrics are
              simulated based on historical games since 2018. Goals predictions
              use Poisson variables representing average goals scored/conceded
              relative to opponent defensive coefficients.
            </div>
          </div>
        </div>
