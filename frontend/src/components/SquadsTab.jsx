...
export default function SquadsTab({
  ...
  const activeTeamName =
    selectedTeam && squadsData[selectedTeam] ? selectedTeam : "Argentina";
  ...
  const squadStats = useMemo(() => {
    ...
    const veteran = [...players].sort(
      (a, b) => (b.caps || 0) - (a.caps || 0),
    )[0];
  ...
  const filteredPlayers = useMemo(() => {
    return players.filter((p) => {
      ...
      const clubMatch =
        p.club && p.club.toLowerCase().includes(playerSearch.toLowerCase());
  ...
  const getPositionBadge = (pos) => {
    let baseClass =
      "px-2 py-0.5 rounded text-[11px] font-bold border tracking-wider uppercase inline-block ";
    if (pos === "GK")
      return (
        <span
          className={baseClass + "text-red-400 bg-red-950/20 border-red-800/30"}
        >
          GK
        </span>
      );
    if (pos === "DF")
      return (
        <span
          className={
            baseClass + "text-blue-400 bg-blue-950/20 border-blue-800/30"
          }
        >
          DF
        </span>
    ...
    if (pos === "FW")
      return (
        <span
          className={baseClass + "text-gold bg-gold-muted border-gold-border"}
        >
          FW
        </span>
      );
    return (
      <span
        className={baseClass + "text-text-muted-alt bg-white/5 border-white/10"}
      >
        {pos || "N/A"}
      </span>
    );
      ...
      <div className="space-y-3">
        ...
        <div className="border-b border-gold/10 pb-4">
          <p className="text-sm text-text-muted-alt font-noto">
            Search, sort, and inspect player profiles, club representations, and
            team compositions.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:hidden relative">
          ...
          >
            <span className="flex items-center gap-3 font-noto">
              ...
              />
              <span className="font-bebas text-lg tracking-wider text-white-alt uppercase">
                {activeTeamName}
              </span>
            ...
            <ChevronDown
              className={`w-4 h-4 text-gold transition-transform duration-200 ${isMobileSelectorOpen ? "rotate-180" : ""}`}
            />
          </button>
              ...
              <div className="absolute left-0 right-0 mt-2 bg-[#101010] border border-gold-border rounded-xl shadow-2xl z-40 max-h-96 overflow-y-auto flex flex-col divide-y divide-white/5 scale-up">
                ...
                <div className="py-1">
                  {filteredTeams.length === 0 ? (
                    <div className="p-4 text-center text-xs text-text-muted-alt font-noto">
                      No teams found.
                    </div>
                  ) : (
                    filteredTeams.map((t) => {
        ...
        <div className="hidden lg:block lg:col-span-4 bg-dark-card border border-white/5 rounded-xl p-4 space-y-4 shadow-lg sticky top-20">
          <div className="space-y-1">
            <h3 className="font-bebas text-lg tracking-wider text-gold">
              SELECT TEAM
            </h3>
            <p className="text-xs text-text-muted-alt font-noto">
              Choose a nation to load their official roster.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-1 border-b border-white/5 pb-3 font-noto">
            {["All", "UEFA", "CONMEBOL", "CONCACAF", "CAF", "AFC", "OFC"].map(
              (c) => (
                <button
                  key={c}
                  onClick={() => setConfedFilter(c)}
          ...
          <div className="max-h-[500px] overflow-y-auto space-y-1 pr-1 custom-scrollbar">
            {filteredTeams.length === 0 ? (
              <div className="text-center py-6 text-xs text-text-muted-alt font-noto">
              ...
              filteredTeams.map((t) => {
                ...
                return (
                  <button
                    ...
                    style={
                      isSelected ? {} : { borderLeft: `3px solid ${t.color}` }
                    }
                  >
                    <span className="flex items-center gap-2.5 font-noto">
                      <TeamFlag
                        teamName={t.name}
                        className="w-5.5 h-3.5 shadow-sm"
                      />
          ...
          >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10 border-b border-white/5 pb-4">
              <div className="flex items-center gap-4">
                <TeamFlag
                  teamName={activeTeamName}
                  className="w-14 h-10 shadow-md object-cover rounded border border-white/10"
                />
                <div className="space-y-0.5">
                  ...
                  <div className="flex items-center gap-2 text-sm text-text-muted-alt font-noto">
                    <span
                      className="px-1.5 py-0.5 rounded text-xs font-bold text-white-alt uppercase"
                      style={{ backgroundColor: activeTeamColor }}
                    >
                      {activeTeamConfed}
                    </span>
                    <span>•</span>
                    <span className="font-semibold text-gold font-noto">
              ...
              <div className="flex items-center gap-3 bg-[#0a0a0a] border border-white/5 rounded-xl px-4 py-2">
                ...
                <div className="text-left font-noto">
                  <div className="text-xs text-text-muted-alt uppercase font-bold tracking-wider">
                    HEAD COACH
                  </div>
                  <div
                    className="text-base font-semibold text-white-alt truncate max-w-[160px]"
                    title={coachName}
                  >
                    {coachName}
                  </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5 mt-5 font-noto text-center">
              <div className="bg-[#0c0c0c] border border-white/5 p-3 rounded-lg flex flex-col justify-center">
                <span className="text-[11px] text-text-muted-alt font-bold uppercase tracking-wider">
                  ROSTER SIZE
                </span>
                <span className="text-2xl font-bebas text-white-alt mt-1 leading-none">
              ...
              <div className="bg-[#0c0c0c] border border-white/5 p-3 rounded-lg flex flex-col justify-center">
                ...
                <span className="text-2xl font-bebas text-gold mt-1 leading-none">
                  {squadStats.avgAge} Yrs
                </span>
              </div>
              ...
              <div className="bg-[#0c0c0c] border border-white/5 p-3 rounded-lg flex flex-col justify-center">
                ...
                <span className="text-2xl font-bebas text-white-alt mt-1 leading-none">
                  {squadStats.totalCaps.toLocaleString()}
                </span>
              </div>
              ...
              <div className="bg-[#0c0c0c] border border-white/5 p-3 rounded-lg flex flex-col justify-center">
                ...
                <span className="text-2xl font-bebas text-white-alt mt-1 leading-none">
                  {squadStats.totalGoals.toLocaleString()}
                </span>
              </div>
            </div>
                  ...
                  <div className="space-y-1 text-white-alt/85 text-xs font-noto">
                    {squadStats.veteran && (
                      <div className="flex justify-between">
                        <span>
                          Most Caps:{" "}
                          <span className="font-semibold text-white-alt">
                            {squadStats.veteran.name}
                          </span>
                        </span>
                        <span className="font-mono text-text-muted-alt font-bold tabular-nums pr-1">
                          ({squadStats.veteran.caps} caps)
                        </span>
                      </div>
                    )}
                    {squadStats.scorer && (
                      <div className="flex justify-between">
                        <span>
                          Top Scorer:{" "}
                          <span className="font-semibold text-white-alt">
                            {squadStats.scorer.name}
                          </span>
                        </span>
                        <span className="font-mono text-text-muted-alt font-bold tabular-nums pr-1">
                          ({squadStats.scorer.goals} goals)
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    {squadStats.topClubs.length > 0 ? (
                      squadStats.topClubs.map((club, idx) => (
                        <div key={club.name} className="flex justify-between">
                          ...
                          <span className="font-semibold text-white-alt shrink-0 font-mono tabular-nums pr-1">
                            {club.count} {club.count > 1 ? "players" : "player"}
                          </span>
                        </div>
                      ))
                    ) : (
                      <div className="text-xs text-text-muted-alt italic">
                        Club breakdown unavailable.
                      </div>
                    )}
                  </div>
                </div>
                ...
                <tbody className="text-sm divide-y divide-[#141414]/80">
                  {sortedPlayers.length === 0 ? (
                    <tr>
                      <td
                        colSpan={7}
                        className="text-center py-10 text-text-muted-alt italic"
                      >
